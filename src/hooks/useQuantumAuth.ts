...
export function useQuantumAuth() {
    ...
    const restoreStoredSession = useCallback(async () => {
        ...
        try {
            ...
            if (!isStoredAuthSessionFresh(storedSession)) {
                ...
                setAuthStatus("guest");
                if (storedSession) {
                    setAuthNotice("Your session expired. Sign in again to sync.");
    ...
    useEffect(() => {
        ...
        const timeout = setTimeout(async () => {
            ...
            setSession(null);
            setAuthStatus("guest");
            setAuthNotice("Your account session expired. Sign in again to sync.");
            void clearStoredAuthSession();
        }, delay);
    ...
    const signIn = useCallback(async () => {
        ...
        try {
            ...
            setAuthStatus("authenticated");
            setAuthNotice("Signed in to your account.");
        } catch (error) {
            authPromptActiveRef.current = false;
            if (!isCurrentAuthOperation(operationRef, operationId)) return;
            setAuthStatus(session ? "authenticated" : "guest");
